...
public static class AbilityUpg
{
    ...
    public static readonly System.Collections.Generic.Dictionary<ModType, Path[]> Mods = new()
    {
        ...
        [ModType.FrostNova] = new[] { new Path("Coldsnap", "+nova damage"), new Path("Whiteout", "+radius"), new Path("Deep Freeze", "+freeze buildup & freeze duration"), new Path("Bitter Cold", "+1 delayed after-pulse", true), new Path("Flash Freeze", "the nova freezes foes instantly", true) },
        ...
        [ModType.Smite]      = new[] { new Path("Verdict", "+damage"), new Path("Far Reach", "+target range"), new Path("Retribution", "+slow & self-heal"), new Path("Condemn", "smited foes are marked — vulnerable to +damage", true), new Path("Wrath of Heaven", "+1 smite target", true) },
        ...
        // (DOOM kit) betray · spread · focus. None of them detonate — only the charged release does that.
        [ModType.Turncoat] = new[] { new Path("Barbs", "+Doom banked on both"), new Path("Wide Pin", "+reach for its victim"), new Path("Deep Pin", "+how long it fights for you"), new Path("Crossed Pins", "the betrayal lasts longer", true), new Path("Pincushion", "whoever it hits turns too", true) },
        [ModType.Fray]     = new[] { new Path("Copy", "+share of the bank passed on"), new Path("Reach", "+radius"), new Path("Spread", "+foes it reaches"), new Path("Wildfire", "+foes again", true), new Path("Snarl", "the snapping threads root what they touch", true) },
        [ModType.Effigy]   = new[] { new Path("Sympathy", "+share of your damage banked on it"), new Path("Nail", "+opening Doom"), new Path("Binding", "+duration"), new Path("Bloodline", "+share again", true), new Path("Reckoning", "when the effigy ends, everything banked on it goes off", true) },
        [ModType.ArcStorm]    = new[] { new Path("Voltage", "+per-bolt damage"), new Path("Arc", "+jump count"), new Path("Conductance", "+fork range"), new Path("Overcharge", "+1 chain jump", true), new Path("Chain Reaction", "struck foes are conduit-marked & take amplified damage", true) },
    ...
    public static readonly System.Collections.Generic.Dictionary<FinType, Path[]> Fins = new()
    {
        ...
        [FinType.Lance] = new[] { new Path("Judgement", "+lance damage"), new Path("Legion", "+lance count"), new Path("Wide Aim", "+per-lance radius"), new Path("Condemn", "struck foes are rooted", true), new Path("Rain of Heaven", "+1 delayed volley", true) },
        ...
        // (DOOM kit) all three ADD Doom rather than spending it — the charged release is the only thing that sets it off.
        [FinType.DanseMacabre] = new[] { new Path("Fervor", "+Doom applied & +damage their blows bank"), new Path("Ballroom", "+radius"), new Path("Tempo", "+how long they fight each other"), new Path("Encore", "the dance lasts longer and spreads", true), new Path("Grand Finale", "when the music stops, every surviving dancer's Doom goes off at once", true) },
        [FinType.Rout]         = new[] { new Path("Terror", "+flee duration"), new Path("Stampede", "+radius"), new Path("Trample", "+Doom applied"), new Path("Contagious Panic", "+flee duration again — the panic spreads", true), new Path("Scattered", "fleeing foes lose you entirely, not just their nerve", true) },
        [FinType.CurtainCall]  = new[] { new Path("Obeisance", "+Doom applied"), new Path("Stage", "+reach & arc"), new Path("Held", "+kneel duration"), new Path("Exposed", "kneeling foes eat an automatic crit", true), new Path("Last Rites", "the cone DETONATES every bank it touches instead of feeding them", true) },
        ...
        // NOTE: Overcharge is a FLAT crit bonus, not a ramp — Arcane Torrent is one instantaneous sweep, not a channel.
        // The old "crit ramps as it fires" text was copied from Spelllance's Overload (which IS a real ramp, see UpdateBeam).
        [FinType.ArcaneBlast] = new[] { new Path("Surge", "+damage"), new Path("Breadth", "+width"), new Path("Distance", "+length & knockback"), new Path("Overcharge", "+crit chance & crit damage", true), new Path("Cataclysm", "chains through conduit-marked foes — needs a conduit source (Conduit Swarm / Chain Reaction / Arcane witch)", true) },
...
public static class UpgradePool
{
    ...
    // (dev/ai) Build every card the pool can currently produce for the LIVE player, at every rarity each def supports.
    // The card_pool_audit scenario uses this to confirm no witch is short-changed on her own affinity cards — the defs
    // gate themselves on Game.I.Player, so switching witch and re-running this is the only honest way to count them.
    public static List<UpgradeCard> DebugPool()
    {
        if (_defs == null) Build();
        var outl = new List<UpgradeCard>();
        foreach (var d in _defs)
            foreach (var r in d.Rars)
            {
                UpgradeCard c = null;
                try { c = d.Make(r, Rarities.Mag(r)); } catch { }
                if (c != null) outl.Add(c);
            }
        return outl;
    }
    ...
    private static void Build()
    {
        _defs = new List<UpgradeDef>
        {
            ...
            Def(EpicP, (r,m)=>{ var pl=Game.I?.Player; if (pl!=null && pl.CrimsonWitch) return WitchCard(r,"Bloodrush","+16% damage while your combo sits at its cap", p=>p.SitComboCap+=0.16f); float add=r==Rarity.Legendary?1f:0.5f; return Card(r,"Deep Reserve",$"+{add:0.0} max mana (max 5)", p=>{ if(p.S.ManaMax<5f){p.S.ManaMax=Mathf.Min(5f,p.S.ManaMax+add); p.Mana=Mathf.Min(p.S.ManaMax,p.Mana+add);} }); }),
            ...
            // ---- finishers (Q/E/F) ----
            // (RETIRED — DOOM REWORK) Hex Pulse / Soul Reap / Hex Chains / Doom Sigil / Hex Mark / Cursefield.
            // The Forsaken kit was rebuilt around Doom; these six were the old tether/Mark abilities it replaces.
            // Their FinType/ModType cases stay in Player.ExecuteFinisher + ApplyChargedMods for save/state compat
            // (same precedent as Fullmod and Witch's Hollow), but they are no longer rollable, sellable or
            // upgradable — every roll path gates on AbilityUpg, and their tree entries are gone too.
            ...
            // (REMOVED) Witching Hour (Fullmod) + Witch's Hollow (HexField) — neither was ever converted to the stack-driven
            // upgrade tree, so every roll path (which gates on AbilityUpg.IsFin) already skipped these defs. Witch's Hollow
            // was still reachable via the scroll vendor and arrived permanently un-upgradable; that hole is closed in
            // Game.BuildScrollOffer. Their FinType cases in Player.ExecuteFinisher stay for save/state compat.
            ...
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.VerdantWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Deepening Grove", $"+{Mathf.RoundToInt(16*m)}% damage while 3 or more tree-ents stand with you", p=>p.SitGrove+=0.16f*m); }),
            ...
            Def(AllR,       (r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.VerdantWitch || pl.KillRefreshCombo) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Quick Roots", "every kill re-opens your combo window — and your Grove grows on combo, so the forest never stops", p=>p.KillRefreshCombo=true); }),
            // --- Gale affinity (wind / mobility / knockback) (NEW) ---
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.GaleWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Slipstream",$"+{Mathf.RoundToInt(18*m)}% damage for 4s after you dash — never stop moving", p=>p.SitPostDash+=0.18f*m); }),
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.GaleWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Crosswind",$"+{Mathf.RoundToInt(16*m)}% damage while your feet are off the ground", p=>p.SitAirborne+=0.16f*m); }),
            ...
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.FrostWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Rime",$"+{Mathf.RoundToInt(18*m)}% damage while you hold your ground (1s after you stop moving)", p=>p.SitStill+=0.18f*m); }),
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.FrostWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Lingering Frost",$"+{Mathf.RoundToInt(15*m)}% damage while your mana is full — patience, then the shot", p=>p.SitFullMana+=0.15f*m); }),
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.FrostWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Brittle",$"+{Mathf.RoundToInt(15*m)}% damage while your health is above 85% — untouched and precise", p=>p.SitHighHp+=0.15f*m); }),
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.FrostWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Shatterpoint",$"+{Mathf.RoundToInt(16*m)}% damage while your combo sits at its cap", p=>p.SitComboCap+=0.16f*m); }),
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.FrostWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Splinterfrost",$"every critical hit returns {(0.12f*m):0.00} mana", p=>p.CritMana+=0.12f*m); }),
            ...
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.ForsakenWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Wasting Curse",$"+{Mathf.RoundToInt(18*m)}% damage while you hold your ground (1s after you stop moving)", p=>p.SitStill+=0.18f*m); }),
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.ForsakenWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Deepening Hex",$"+{Mathf.RoundToInt(17*m)}% damage while your health is below 40% — feed on your own ruin", p=>p.SitLowHp+=0.17f*m); }),
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.ForsakenWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Leeching Beam",$"every kill mends you for {Mathf.RoundToInt(3*m)}% of your max health", p=>p.KillHeal+=0.03f*m); }),
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.ForsakenWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Sympathetic Pain",$"+{Mathf.RoundToInt(15*m)}% damage while your mana is full", p=>p.SitFullMana+=0.15f*m); }),
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.ForsakenWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Virulent Hex",$"+{Mathf.RoundToInt(16*m)}% damage while your combo sits at its cap", p=>p.SitComboCap+=0.16f*m); }),
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.ForsakenWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Binding Ritual",$"every critical hit returns {(0.12f*m):0.00} mana", p=>p.CritMana+=0.12f*m); }),
            ...
            // (CARD REWORK) These used to hand out crescent pierce / Lunar damage / ult charge — the exact fields her perk
            // tree already feeds, so the two just stacked. Cards now pay out on a CONDITION instead; see Player.SitMul.
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || pl.WitchIndex!=0 || pl.KillRefreshCombo) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Reaper's Rhythm","every kill re-opens your combo window — keep the chain alive by killing, not by hurrying", p=>p.KillRefreshCombo=true); }),
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || pl.WitchIndex!=0) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Nightfall's Gift",$"+{Mathf.RoundToInt(14*m)}% damage while it is NIGHT", p=>p.SitNight+=0.14f*m); }),
            Def(LegP,       (r,m)=>{ var pl=Game.I?.Player; if (pl==null || pl.WitchIndex!=0) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Lunar Eclipse","+30% damage at night, and your kills re-open the combo window", p=>{ p.SitNight+=0.3f; p.KillRefreshCombo=true; }); }),
            // --- Divine affinity (holy support / smite) ---
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.DivineWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Unbroken Faith",$"+{Mathf.RoundToInt(15*m)}% damage while your health is above 85% — stay pristine", p=>p.SitHighHp+=0.15f*m); }),
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.DivineWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Steadfast Light",$"+{Mathf.RoundToInt(16*m)}% damage while you hold your ground (1s after you stop moving)", p=>p.SitStill+=0.16f*m); }),
            ...
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.CrimsonWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Crimson Communion",$"every kill mends you for {Mathf.RoundToInt(3*m)}% of your max health", p=>p.KillHeal+=0.03f*m); }),
            ...
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.EmberWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Kindling",$"+{Mathf.RoundToInt(16*m)}% damage while your combo sits at its cap", p=>p.SitComboCap+=0.16f*m); }),
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.EmberWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Cinderreach",$"+{Mathf.RoundToInt(17*m)}% damage while your health is below 40% — burn brightest at the end", p=>p.SitLowHp+=0.17f*m); }),
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.EmberWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Detonator",$"+{Mathf.RoundToInt(18*m)}% damage for 4s after you dash", p=>p.SitPostDash+=0.18f*m); }),
            Def(RareP,      (r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.EmberWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Pyre Heart","every kill mends you for 4% of your max health — the fire feeds you", p=>p.KillHeal+=0.04f); }),
            Def(LegP,       (r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.EmberWitch || pl.EmberInferno) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Living Inferno","the fire never gutters: +30% damage at a capped combo, and every kill re-opens the combo window", p=>{ p.EmberInferno=true; p.SitComboCap+=0.3f; p.KillRefreshCombo=true; }); }),
            // --- Arcane affinity + legendaries (NEW) ---
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.ArcaneWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Amplifier",$"+{Mathf.RoundToInt(18*m)}% damage for 4s after you blink — strike out of the step", p=>p.SitPostDash+=0.18f*m); }),
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.ArcaneWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Feedback Loop",$"every critical hit returns {(0.12f*m):0.00} mana", p=>p.CritMana+=0.12f*m); }),
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.ArcaneWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Overcharged Core",$"+{Mathf.RoundToInt(15*m)}% damage while your mana is full", p=>p.SitFullMana+=0.15f*m); }),
            Def(RareP,      (r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.ArcaneWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Arcane Focus","+16% damage while your combo sits at its cap", p=>p.SitComboCap+=0.16f); }),
            Def(LegP,       (r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.ArcaneWitch || pl.ArcaneLiving) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Living Current","raw power courses through you: +35% damage at full mana, and every crit returns 0.25 mana — keep the reservoir brimming", p=>{ p.ArcaneLiving=true; p.SitFullMana+=0.35f; p.CritMana+=0.25f; }); }),
            ...
            Def(new[]{Rarity.Common,Rarity.Uncommon,Rarity.Rare,Rarity.Epic}, (r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.ArcaneWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Lingering Sigils",$"+{Mathf.RoundToInt(15*m)}% damage while your health is above 85%", p=>p.SitHighHp+=0.15f*m); }),
            // --- more Lunar affinity (it only had 2 non-legendary picks) ---
            Def(RareP,      (r,m)=>{ var pl=Game.I?.Player; if (pl==null || pl.WitchIndex!=0) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Waxing Gibbous","+20% damage for 4s after you dash — step through, then strike", p=>p.SitPostDash+=0.2f); }),
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || pl.WitchIndex!=0) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Moonglow",$"+{Mathf.RoundToInt(15*m)}% damage while your health is above 85%", p=>p.SitHighHp+=0.15f*m); }),
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || pl.WitchIndex!=0) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Silver Crescent",$"every critical hit returns {(0.12f*m):0.00} mana", p=>p.CritMana+=0.12f*m); }),
            ...
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.DivineWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Martyr's Resolve",$"+{Mathf.RoundToInt(17*m)}% damage while your health is below 40% — the light burns hottest in extremis", p=>p.SitLowHp+=0.17f*m); }),
            Def(UncRareEpic,(r,m)=>{ var pl=Game.I?.Player; if (pl==null || !pl.DivineWitch) return new UpgradeCard { Rarity=r, Hidden=true }; return WitchCard(r,"Seraph's Grace",$"every kill mends you for {Mathf.RoundToInt(3*m)}% of your max health", p=>p.KillHeal+=0.03f*m); }),
            ...
            // (DOOM kit) witch-agnostic: each banks its own Doom, which detonates on the fuse for anyone who isn't her.
            Def(ComUncRare, (r,m)=>ModCard(r,ModType.Turncoat, m, "turn the foe you hit — it attacks the nearest enemy once, with its own attack")),
            Def(UncRareEpic,(r,m)=>ModCard(r,ModType.Fray,     m, $"snap the threads: copy half the struck foe's Doom onto everything within {Mathf.RoundToInt(6f+m)} units")),
            Def(UncRareEpic,(r,m)=>ModCard(r,ModType.Effigy,   m, "nail your doll to the foe you hit — a share of all damage you deal elsewhere is banked on it")),
            ...
            // (DOOM kit) witch-agnostic like every finisher: each applies its own Doom, so a non-Forsaken build still
            // gets bombs going off on the fuse without owning her channel.
            Def(ComUncRare, (r,m)=>FinCard(r,FinType.DanseMacabre, r==Rarity.Common?8:(r==Rarity.Uncommon?7:6), 0.9f+m*0.22f, "aimed: doom the area, then the doomed turn on each other")),
            Def(ComUnc,     (r,m)=>FinCard(r,FinType.Rout,         r==Rarity.Common?8:6, 0.9f+m*0.22f, "doom everything around you and send it fleeing — buy yourself room")),
            Def(UncRareEpic,(r,m)=>FinCard(r,FinType.CurtainCall,  r==Rarity.Uncommon?7:(r==Rarity.Rare?6:5), 0.9f+m*0.24f, "a cone of dread: the heaviest Doom load, and the doomed kneel where they stand")),
    ...
    private static List<UpgradeDef> WitchLadder()
    {
        ...
        Witch(p => p.GaleWitch,         "Wind",   "Gathering Gale", "Cutting Gust",   "Windward",         "+move speed", (p, m) => p.S.Speed = Mathf.Min(16.5f, p.S.Speed * (1 + 0.03f * m)));   // NOT "Slipstream" — that title belongs to her affinity card, and Banned keys on Title
        ...
        Witch(p => p.EmberWitch,        "Ember",  "Emberglow",      "Scorch",         "Wildfire Spread",  "+area",       Area);   // NOT "Kindling" — that title belongs to her affinity card
